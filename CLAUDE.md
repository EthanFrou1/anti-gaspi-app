...
- **Profil** (privé, jamais montré aux autres membres) : temps de cuisine souhaité, budget par repas (tranches), régime, exclusions d'ingrédients, allergies, objectif (équilibré, prise de muscle, repas légers, anti-gaspi simple), nombre de portions par défaut, **goûts** (aliments « Pas pour moi », réglage « Pas épicé »). Uniquement des listes fermées, **aucun texte libre** (il finirait dans le prompt de l'IA : risque d'injection de prompt).
- **Goûts** : 32 aliments courants (`DislikedFood`), sans doublon avec les exclusions ; des goûts, pas des données de santé (aucun consentement). Absents de l'onboarding : proposés une fois avant la première recette, modifiables dans Profil.
...
- **Repas partagé** : quand plusieurs membres mangent ensemble, les contraintes de leurs profils se combinent (si l'un est végétarien, la recette l'est ; les allergies de tous sont exclues ; les goûts de tous aussi, et « Pas épicé » dès qu'un convive le coche).
...
- **Données de profil et IA** : indiquer dans la politique de confidentialité que les contraintes alimentaires (dont les allergies, avec consentement) sont transmises au fournisseur d'IA pour générer les recettes, sans identité ; de même pour les contraintes d'un repas saisies pour des invités (sans nom, jamais enregistrées par l'app) ; prévoir l'export des données personnelles (droit d'accès).
...
- **Invités** (hors du foyer, sans nom ni profil) : jusqu'à 10, une portion chacun, 12 personnes au plus à table. **Contraintes pour ce repas** (liste fermée : végétarien, sans porc, pas épicé, sans fruits à coque, sans arachides, sans gluten, sans lactose = aucun produit laitier) : ajoutées aux profils des convives pour cette génération seulement, **jamais enregistrées ni journalisées** (ni dans la recette, ni dans « Non utilisés » ; un « Pas épicé » des seuls invités compte comme une contrainte). Des produits du frigo écartés pour une contrainte (allergène, exclusion) ne sont jamais nommés : seule la réponse de génération porte la mention générique « Certains produits du frigo ont été écartés pour respecter les contraintes des convives », que l'app affiche sans l'enregistrer.
- **Allergènes et porc, vérifiés par le code** comme les goûts : fruits à coque, arachides, gluten et lait (qu'ils viennent d'un profil ou des invités) et l'exclusion « porc » ont leurs mots-clés et exceptions (noix de coco, noix de beurre, pâtes sans gluten, lait de coco, jambon de dinde…). C'est un filet de sécurité, pas une garantie : l'app renforce l'avertissement « vérifie les étiquettes », et aucune recette n'est présentée comme « sans gluten ». Un rejet pour une contrainte est journalisé « contrainte du repas », sans dire laquelle.
- **Goûts des convives, vérifiés par le code** (`FoodPreferences`) : chaque aliment a des mots-clés (mots entiers, sans accents, singulier ou pluriel, ingrédients composés comme « pesto » ou « ketchup », exceptions comme « huile d'olive » ou « haricot coco », mentions négatives « sans ail » ignorées). Avant l'IA, les produits du frigo concernés sont écartés ; la recette cite les 5 plus urgents (« Non utilisés : champignons (préférences d'un convive) », sans dire lequel). Après l'IA, une recette qui contient un aliment écarté est rejetée (503, quota non décompté) ; ces rejets sont journalisés à part (événement `RecipeRejectedByPreferences`, aliment seul, sans identité) pour mesurer leur fréquence.
...
- [ ] Harissa dans le frigo, 1 invité et « Pas épicé » : la recette n'en contient pas, la mention générique « Certains produits du frigo ont été écartés… » s'affiche (sans « Non utilisé : Harissa ») ; rouverte depuis l'historique, plus aucune mention.
